...
Fake4Dataverse uses an **undo log** rather than full environment snapshots. During a transaction, every Create, Update, and Delete operation records its inverse:

| Operation | Undo Action |
|-----------|-------------|
| Create | Delete the created record |
| Update | Restore the record's pre-update state |
| Delete | Re-insert the deleted record |

On failure, the undo log is replayed in reverse order. This approach is **concurrency-safe**: rolling back one transaction only undoes that transaction's changes, leaving concurrent transactions' writes intact.
...
If a **PostOperation** plugin throws an exception, the entire core mutation is rolled back — the record is restored to its state before the operation began. Fake4Dataverse replicates this behavior using the same undo-log mechanism used by `ExecuteTransactionRequest`.
...
When an operation runs inside an `ExecuteTransactionRequest`, the implicit transaction scope detects the outer transaction and reuses it — no nested undo log is created. A failure still propagates to the outer transaction which handles rollback of all operations in the batch.
...
| Transaction rollback | SQL transaction rollback | Undo log replay |
| Implicit transaction on CRUD | SQL transaction wrapping pipeline | Undo log wrapping pipeline |
| Concurrent transaction isolation | SQL Server locking | Undo log per-transaction |
