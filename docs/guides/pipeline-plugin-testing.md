...
// Pre-image: entity state before the update
reg.AddPreImage("PreImage", "name", "revenue");

// Post-image: entity state after the update
reg.AddPostImage("PostImage", "name", "revenue");
```

Pass no attribute names to include all attributes:
...
> **Note:** Fake4Dataverse executes asynchronous steps synchronously in-process. `SetAsynchronous()` only sets the `Mode` property so your plugin logic can branch on it.
...
// result.Skipped contains any attributes that could not be mapped (e.g. workflow activities)
```

This is particularly useful when your plugin project already uses SPKL-style attributes for deployment — you get test registration for free, matching your production configuration.
