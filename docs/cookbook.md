...
Runnable sample files are split by project type with separate test projects:

- `samples/Fake4Dataverse.Samples.AccountService/AccountService.cs`
- `samples/Fake4Dataverse.Samples.AccountService.Tests/AccountServiceTests.cs`
...
- `ITracingService` — traces are captured and readable via `env.Pipeline.Traces`
...
public void UserWithoutCreatePrivilege_CannotCreateAccount()
{
    ...
    env.Security.AddRole(userId, "ReadOnly");
    env.Security.AddPrivilege("ReadOnly", "account",
...
public void OperationLog_RecordsAllCalls()
{
    ...
    // Check specific operations on the per-session log
    ...
    Assert.True(env.OperationLog.HasCreated("account", id));
}
```
...
For file-based seed setup, use the built-in `Fake4Dataverse.DataProviders` extensions:
...
## 11. Using the FakeItEasy Adapter Package
...
// Verify interaction with FakeItEasy while all behavior is backed by Fake4Dataverse
A.CallTo(() => fake.Create(A<Entity>.Ignored))
    .MustHaveHappenedOnceExactly();
```

For plugin scenarios that require an `IOrganizationServiceFactory` fake:

```csharp
var factory = service.AsFakeFactory();
var orgService = factory.CreateOrganizationService(Guid.NewGuid());
```

## 12. Using the AwesomeAssertions Companion Package
...
## 13. Using the Shouldly Companion Package
