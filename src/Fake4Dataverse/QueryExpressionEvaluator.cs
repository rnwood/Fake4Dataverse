            ...
            foreach (var order in orders)
            {
                if (ordered == null)
                {
                    ordered = order.OrderType == OrderType.Ascending
                        ? entities.OrderBy(e => e.Contains(order.AttributeName) ? e[order.AttributeName] as IComparable : null)
                        : entities.OrderByDescending(e => e.Contains(order.AttributeName) ? e[order.AttributeName] as IComparable : null);
                }
                else
                {
                    ordered = order.OrderType == OrderType.Ascending
                        ? ordered.ThenBy(e => e.Contains(order.AttributeName) ? e[order.AttributeName] as IComparable : null)
                        : ordered.ThenByDescending(e => e.Contains(order.AttributeName) ? e[order.AttributeName] as IComparable : null);
                }
            }
